...
struct ScrapCacheEntry {
    ...
    uint32_t last_used_tick;        // Last access timestamp
...
struct EnhancedMemoryPool {
    ...
    uint32_t last_reset_tick;          // Last pool reset timestamp
...
static inline uint32_t GetSizeClass(size_t size) {
    if (size == 0) return 0;
...
// Pool utilities
static inline SIZE_T GetPoolUsed(const EnhancedMemoryPool* pool) {
    return (SIZE_T)pool->used;
}

static inline bool IsInPool(const void* ptr, const EnhancedMemoryPool* pool) {
...
// Segment utilities
static inline bool IsInCustomHeap(const void* ptr) {
...
static bool InitializeScrapCache() {
    ...
    __try {
        InitializeCriticalSectionAndSpinCount(&g_scrap_cache.cache_lock, HEAP_SPIN_COUNT);
...
static bool ScrapCacheReleaseBuffer(void* ptr, size_t size) {
    ...
    __try {
        ...
        if (g_scrap_cache.entry_count < SCRAP_CACHE_ENTRIES) {
            ...
            LONG lru_index = 0;
            DWORD oldest_tick = g_scrap_cache.entries[0].last_used_tick;
...
static bool InitializeCustomHeap() {
    ...
    __try {
        InitializeCriticalSectionAndSpinCount(&g_custom_heap.heap_lock, HEAP_SPIN_COUNT);
...
static MemorySegment* CreateMemorySegment() {
    ...
    __try {
        InitializeCriticalSectionAndSpinCount(&segment->segment_lock, HEAP_SPIN_COUNT);
    }
    __except(EXCEPTION_EXECUTE_HANDLER) {
...
static bool InitializeEnhancedPool(EnhancedMemoryPool* pool, SIZE_T size, const char* name, uint32_t pool_id) {
    ...
    pool->last_reset_tick = GetTickCount();
    pool->active = 1;
    
    __try {
        InitializeCriticalSectionAndSpinCount(&pool->pool_lock, HEAP_SPIN_COUNT);
        
        LOG_INFO("%s initialized: %zu MB at 0x%p (large pages: %s)", 
                 name, size / (1024*1024), pool->base,
                 (pool->base) ? "attempt made" : "not attempted");
        return true;
    }
    __except(EXCEPTION_EXECUTE_HANDLER) {
...
static void* EnhancedPoolAllocate(size_t size) {
    ...
    if (offset + total_size > pool->size) {
        LOG_ERROR("Pool overflow detected after atomic allocation");
        return NULL;
    }
...
static bool InitializeCriticalSections() {
    ...
    __try {
        InitializeCriticalSectionAndSpinCount(&g_stats_lock, HEAP_SPIN_COUNT);
        InitializeCriticalSectionAndSpinCount(&g_log_lock, HEAP_SPIN_COUNT);
        InitializeCriticalSectionAndSpinCount(&g_system_lock, HEAP_SPIN_COUNT);
        
        // Initialize heap critical sections array
        for (int i = 0; i < 8; i++) {
            InitializeCriticalSectionAndSpinCount(&g_heap_critical_sections[i], HEAP_SPIN_COUNT);
        }
...
// Apply budget patches (placeholder - implement if needed)
    ...
    LOG_INFO("Multi-tier system active: %.2f GB total managed memory", 
